...
(function (global) {
  ...
  function _bindDesktopIcons() {
    ...
    document.getElementById('tb-lock-btn')?.addEventListener('click', () => {
  ...
  function _bindDesktopContext() {
    document.getElementById('desktop')?.addEventListener('contextmenu', e => {
      ...
      _showContextMenu(e.clientX, e.clientY, [
        ...
        { sep: true },
        { sep: true },
        { label: '&#128274; Lock Screen',      action: () => global.NovaOS.LockScreen?.lock() },
  ...
  function _initPWAPrompt() {
    ...
    window.addEventListener('beforeinstallprompt', e => {
      ...
      setTimeout(() => _showInstallBanner(deferredPrompt), 3000);
    });
  }

  function _showInstallBanner(prompt) {
    ...
    banner.innerHTML = `
      ...
      <div class="pwa-text">
        <div class="pwa-title">Install Nova OS</div>
        <div class="pwa-sub">Add to your home screen for the full experience</div>
      </div>
      <div class="pwa-actions">
        <button class="btn btn-ghost btn-sm" id="pwa-dismiss">Not now</button>
    ...
    banner.querySelector('#pwa-install').addEventListener('click', async () => {
      ...
      if (outcome === 'accepted') {
        global.NovaOS.Kernel.notify('Nova OS', 'App installed successfully!', '⬡', 4000);
      }
    });
